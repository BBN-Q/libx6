...
void X6_1000::silly() {
    //Copy II's code to try and get DAC alignment working
    ...
    // stream_.OnAfterStop.SetEvent(this, &X6_1000::HandleAfterStop);
    // stream_.OnAfterStop.Synchronize();
    ...
    const int RxBmSize = std::max(32/4, 1) * 4;
    const int TxBmSize = std::max(32/4, 1) * 4;
    module_.IncomingBusMasterSize(RxBmSize * Meg);
    module_.OutgoingBusMasterSize(TxBmSize * Meg);
    module_.Target(0);
    ...
    FILE_LOG(logDEBUG) << "Opened module";

    module_.Reset();

    FILE_LOG(logDEBUG) << "Reset module";

    stream_.ConnectTo(&module_);
    FILE_LOG(logDEBUG) << "Stream Connected...";

    prefillPacketCount_ = stream_.PrefillPacketCount();
    FILE_LOG(logDEBUG) << "Stream prefill packet count: " << prefillPacketCount_;

    std::stringstream msg;
    msg << std::hex << "Logic Version: " << module_.Info().FpgaLogicVersion()
        << ", Hdw Variant: " << module_.Info().FpgaHardwareVariant()
        << ", Revision: " << module_.Info().PciLogicRevision()
        << ", Subrevision: " << module_.Info().FpgaLogicSubrevision();
    ...
    msg2 << std::hex << "Board Family: " << module_.Info().PciLogicFamily()
        << ", Type: " << module_.Info().PciLogicType()
        << ", Board Revision: " << module_.Info().PciLogicPcb()
        << ", Chip: " << module_.FpgaName(module_.Info().FpgaChipType());
    FILE_LOG(logDEBUG) << msg2.str();
    
    msg.str("");
    msg << "PCI Express Lanes: " << module_.Debug()->LaneCount();
    if (module_.Debug()->IsGen2Capable())
        msg << " Gen 2";
    else
        msg << " Gen 1 only";
    FILE_LOG(logDEBUG) << msg.str();
    ...
    module_.Clock().ReferenceFrequency(10 * 1e6);
    ...
    msg.str("");
    msg << "Actual PLL Frequencies: [ADC] " << adc_freq_actual << " [DAC] " << dac_freq_actual  ;
    FILE_LOG(logDEBUG) << msg.str();
    ...
    stream_.Disconnect();
    module_.Close();
    FILE_LOG(logDEBUG) << "Stream Disconnected. Device closed.";
}
